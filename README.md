...
Convergence does not imply that all points have merged into one. It is possible for there to be multiple solutions to a quadratic maximisation problem, for example a problem with the single constraint x + y <= 10 will have solutions (10, 0) and (0, 10). In this situation the algorithm will terminate and both of these will be given as optimal points.
